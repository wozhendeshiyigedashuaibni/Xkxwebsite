...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    try {
      ...
      // Table doesn't exist yet - skip rate limiting
      if (e.code === 'P2021') {
        rateLimitEnabled = false;
        console.warn('LoginAttempt table not found - rate limiting disabled. Run: npx prisma db push');
      } else {
        throw e;
      }
    }
    ...
    if (!admin) {
      if (rateLimitEnabled) {
        try { await prisma.loginAttempt.create({ data: { ip: clientIP, success: false } }); } catch {}
      }
      await prisma.$disconnect();
